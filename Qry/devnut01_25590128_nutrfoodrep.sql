...
--SELECT AN, REQDATE, REQEND, REQENDDATE
--FROM NUTR_FOOD_REQS H
--JOIN NUTR_FOOD_REQD D on D.REQID = H.REQID
--WHERE ISNULL(REQEND,'') = 'Y'
--AND (REQDATE BETWEEN '2016-03-01' AND '2016-03-31')
--OR (REQENDDATE BETWEEN '2016-03-01' AND '2016-03-31') 
--UNION
--SELECT AN, REQDATE, REQEND, REQENDDATE
--FROM NUTR_FOOD_REQS
--WHERE ISNULL(REQEND,'') = ''
...
--update NUTR_FOOD_REQS set REQENDDATE = '2016-04-04' where AN = '5300010'

--SELECT R.AN, R.REQDATE, R.REQEND, R.REQENDDATE, R.REQID, D.REQCODE, F.FDES,
...
--WHERE AN = '5200018'--'5300010'
--AND F.FGRC = '0001'
----AND (REQDATE BETWEEN '2016-03-01' AND '2016-03-31')
--GROUP BY R.AN, R.REQDATE, R.REQEND, R.REQENDDATE, R.REQID, D.REQCODE, F.FDES
--ORDER BY R.REQID

--UPDATE NUTR_FOOD_REQS SET REQENDDATE = '2016-03-18' WHERE REQID ='0000C'

--SELECT MIN(REQDATE) AS REQFR, MAX(REQENDDATE) AS REQTO
--FROM NUTR_FOOD_REQS
--WHERE AN = '5300010'

--SELECT R.AN, R.REQDATE, R.REQEND, R.REQENDDATE, R.REQID, D.REQCODE, F.FDES,
--       MIN(R.REQDATE) AS REQFR, MAX(R.REQDATE) AS REQTO
--FROM NUTR_FOOD_REQS R
--JOIN NUTR_FOOD_REQD D ON D.REQID = R.REQID
--JOIN NUTR_FACT F ON F.CODE = D.REQCODE
--WHERE AN = /*'5200018'--'*/'5300010'
...
--(
--SELECT AN, 
--       CASE WHEN REQORD = 1 THEN REQDATE ELSE NULL END AS REQPREV , 
--       CASE WHEN REQORD = 2 THEN REQDATE ELSE NULL END AS REQNOW
--FROM
--(
--SELECT TOP 2 AN, REQID, REQDATE, ROW_NUMBER() OVER(ORDER BY REQID DESC) AS REQORD
--FROM NUTR_FOOD_REQS
--WHERE AN = '5200018' 
--ORDER BY REQID DESC) A
--GROUP BY AN, REQORD, REQDATE) B WHERE REQPREV is not null),
--(select REQNOW
--from
--(
--SELECT AN, 
...
--WHERE AN = '5200018' 
--ORDER BY REQID DESC) A
--GROUP BY AN, REQORD, REQDATE) B WHERE REQPREV is null)
...
--WHERE AN = '5300010'
--AND F.FGRC = '0001'
----AND (REQDATE BETWEEN @FRDATE AND @TODATE)
--AND R.REQDATE BETWEEN (CASE WHEN ISNULL(R.REQEND,'') = 'Y' 
--                            THEN (CASE WHEN @FRDATE > R.REQDATE 
--		                               THEN R.REQDATE ELSE @FRDATE END) 
--							END)  
--GROUP BY R.AN, R.REQDATE, R.REQEND, R.REQENDDATE, R.REQID, D.REQCODE, F.FDES
--ORDER BY R.REQID

--SELECT A.FRDATE , A.TODATE, DATEDIFF(DAY,A.FRDATE, A.TODATE) REQDAYS
--FROM
--(SELECT CASE WHEN  @FRDATE> REQDATE THEN REQDATE ELSE @FRDATE END AS FRDATE,
--        CASE WHEN  @TODATE> ISNULL(REQENDDATE,REQDATE) THEN iSNULL(REQENDDATE,REQDATE) ELSE @TODATE END AS TODATE
--FROM NUTR_FOOD_REQS
--WHERE AN = '5300010') A

--SELECT REQDATE, REQENDDATE
--FROM NUTR_FOOD_REQS
--WHERE AN = '5300010'


--SELECT *, DATEDIFF(DAY,B.REQFR, B.REQTO) AS C_DAYS
--FROM
--(
--SELECT CASE WHEN @FRDATE > A.REQFRDATE THEN A.REQFRDATE ELSE @FRDATE END REQFR,
--       CASE WHEN @TODATE > A.REQTODATE THEN A.REQTODATE ELSE @TODATE END REQTO
--FROM
--(
--SELECT MIN(REQDATE) AS REQFRDATE, MAX(ISNULL(REQENDDATE,REQDATE)) AS REQTODATE
--FROM NUTR_FOOD_REQS
--WHERE AN = '5300010'
--AND ISNULL(REQEND,'') = 'Y') A) B


--goto function FN_REPORT4

select * from dbo.FN_REPORT4('20160226','20160226')

SELECT * FROM dbo.FN_REPORT1('20161226')